...
*                     2001                     *
*      Do not modify this code unless		   *
...
BOOL CreateWindowGL (GL_Window* window)									// This Code Creates Our OpenGL Window
{
	...
	ZeroMemory (window->mouseCoord, sizeof (MouseCoords));
	...
	switch (uMsg)														// Evaluate Window Message
	{
		...
		case WM_KEYUP:													// Update Keyboard Buffers For Keys Released
			if ((wParam >= 0) && (wParam <= 255))						// Is Key (wParam) In A Valid Range?
			{
				...
				window->keys->keyPress = FALSE;
				return 0;												// Return
			}
		break;															// Break
		case WM_MOUSEMOVE:												// Update Mouse movement
			if ((wParam >= 0) && (wParam <= 255))						// 
			{
				window->mouseCoord->x = lParam & 0xFFFF;
				window->mouseCoord->y = (lParam >> 16);
				return 0;												// Return
			}
		break;															// Break
...
int WINAPI WinMain (HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	window.mouseCoord			= &mouseCoord;							// Window MouseCoord structure
	...
	ZeroMemory (&mouseCoord, sizeof (MouseCoords));						// Zero MouseCoord Structure
	...
	while (g_isProgramLooping)											// Loop Until WM_QUIT Is Received
	{
		...
		if (CreateWindowGL (&window) == TRUE)							// Was Window Creation Successful?
		{
			...
			if (Initialize (&window, &keys, &mouseCoord) == FALSE)					// Call User Intialization
			{
				// Failure
				DestroyWindowGL (&window);
				MessageBox(NULL, "Initialization failed. Could not find all resources", "Error", MB_OK | MB_ICONERROR);
				...
				while (isMessagePumpActive == TRUE)						// While The Message Pump Is Active
				{
					...
					else												// If There Are No Messages
					{
						...
						else											// If Window Is Visible
						{
							...
							Update (tickCount - window.lastTickCount, window.hDC);	// Update The Counter

							window.lastTickCount = tickCount;			// Set Last Count To Current Count
							//Draw ();									// Draw Our Scene ... not needed now :D
